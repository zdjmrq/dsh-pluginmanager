...
3. **已安装的 npm 插件包**(profile `node_modules` 里声明 `dsh.bundle.patch` 的包):显示安装/挂载来源,未挂载的一键按包名挂载进补丁层。
...
5. **已安装的 npm 插件包**组:用 `pnpm add <包>` 装进 profile 的包会出现在这里——未挂载的点 **挂载**(按包名进补丁层,热生效),补丁层挂载的点 **卸载**,bundles 全局挂载的会提示去运行树组管理;
